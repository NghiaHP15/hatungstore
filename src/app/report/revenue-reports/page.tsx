...
import { exportInvoiceDailyExcel } from "@/utils/exportInvoiceExcel";
import dayjs from "dayjs";
import { exportInvoiceDailyPdf } from "@/utils/exportInvoicePDF";
...
const ReportPage = () => {
    ...
    const [data, setData] = useState<ReportRevenue[]>([]);
    const debouncedValue = useDebounce(lazyParams.search, 500);

    const fetchReport = async () => {
        try{
            setLoading(true);
            const res = await reportAPI.getRevenue({ limit: lazyParams.limit, search: debouncedValue, status: lazyParams.status, start_date: lazyParams.start_date, end_date: lazyParams.end_date });
    ...
    const column: any = [
        ...
        },
        {
            ...
            title: "Ngày",
            dataIndex: 'day',
            render: (_: any, record: ReportRevenue) => <span className="line-clamp-2">{record.date}</span>
        },
        {
            key: 'revenue',
            dataIndex: 'revenue',
            title: "Số đơn hàng",
            render: (_: any, record: ReportRevenue) => <span className="line-clamp-2">{record.total_orders}</span>
        },
        {
            key: 'cash_revenue',
            dataIndex: 'cash_revenue',
            title: "Tiền mặt",
            render: (_: any, record: ReportRevenue) => <span className="line-clamp-2">{formatCurrency(record.cash_revenue)}</span>
        },
        {
            key: 'transfer_revenue',
            dataIndex: 'transfer_revenue',
            title: "Chuyển khoản",
            render: (_: any, record: ReportRevenue) => <span className="line-clamp-2">{formatCurrency(record.transfer_revenue)}</span>
        },
        {
            key: 'invoice',
            dataIndex: 'invoice',
            title: "Doanh thu",
            render: (_: any, record: ReportRevenue) => <span className="line-clamp-2">{formatCurrency(record.total_revenue)}</span>
        },
    ]
