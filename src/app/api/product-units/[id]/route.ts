...
export async function GET(
  ...
  const { data, error } = await supabase.from("product_units").select("*").eq("id", id).single();
...
export async function PUT(
  ...
  try {
    ...
    // Cập nhật product
    const { data: product_unit, error } = await supabase
      .from("product_units")
      .update({ name, unit_name, price, product_id })
      .eq("id", id)
      .select()
      .single();
    ...
    return NextResponse.json(product_unit);
  } catch (e: any) {
